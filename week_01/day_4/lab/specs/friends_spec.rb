...
class TestFriends < MiniTest::Test
  ...
  def test_get_favorite_show
    result = get_favorite_show(@person3)
  ...
  def test_add_friend
    result = add_friend(@person3, "Scrappy-Doo")
    assert_equal(@person3[:friends].length, 3)
  end
  ...
  def test_loan_money
    result = loan_money(@person5, @person3, 20)
    assert_equal(@person5[:monies], 80)
    assert_equal(@person3[:monies], 40)
  end

  # 8. Find the set of everyone's favourite food joined together
  # (hint: concatenate the favourites/snack arrays together)

  def test_list_favourite_snacks
    result = list_favourite_snacks(@people)
